...
AATAMS_DM_FILE_TYPE_NAMES = ('metadata', 'ctd', 'diag', 'dive', 'haulout',
                             'ssmoutputs', 'summary')
...
def not_applicable(astr):  # TODO: handle every case as particular?
    if astr == '' or astr == 'NA' or astr == 'NaN':
        return ''
    else:
        return False
...
def is_positive(afloat):
    return afloat > 0


def is_negative(afloat):
    return afloat < 0
...
check_number_of_files_is_correct = partial(check_len_list,
                                           AATAMS_DM_NUMBER_OF_FILES_IN_ZIP)
check_name_of_files_is_correct = partial(check_file_names,
                                         AATAMS_DM_FILE_TYPE_NAMES)

# Schema dicts & cases
FILENAMES_IN_ZIP_SCHEMA = And(check_number_of_files_is_correct,
...
DIVE_SCHEMA = {
    ...
    **{'d' + str(x): Or(CSV_EMPTY, CSV_POSITIVE_FLOAT)
       for x in range(1, 5)},  # TODO
    **{'v' + str(x): Or(CSV_EMPTY, CSV_FLOAT)
       for x in range(1, 6)},
    ...
    **{
        ...
        for x in [
            'pca.desc', 'pca.btm', 'pca.asc',\
            'pca.max.desc', 'pca.max.btm', 'pca.max.asc',\
            'pca.mean.desc', 'pca.mean.btm', 'pca.mean.asc'
        ]
    },
    **{
        x: Or(CSV_EMPTY, CSV_POSITIVE_FLOAT)
        for x in [
            'swim.eff.desc', 'swim.eff.btm', 'swim.eff.asc', 'swim.eff.whole'
        ]
    },
    **{
        x: Or(CSV_EMPTY, CSV_POSITIVE_INT)
        for x in [
            'secs.desc', 'secs.btm', 'secs.asc',\
            'pitch.desc', 'pitch.btm', 'pitch.asc'
        ]
    },
    ...
    **{'d' + str(x): Or(CSV_EMPTY, CSV_POSITIVE_FLOAT)
       for x in range(6, 26)},
    **{'t' + str(x): Or(CSV_EMPTY, CSV_POSITIVE_FLOAT)
       for x in range(6, 26)},
...
HAULOUT_SCHEMA = {
    ...
    'end.number': CSV_INT,
    **COORD_SCHEMA, 'cid': METADATA_SCHEMA['sattag_program']
}
...
SSMOUTPUTS_SCHEMA = {
    ...
    **POS_SCHEMA, 'cid': METADATA_SCHEMA['sattag_program']
}
...
class AATAMS_DM_Schema(CSVSchema):
    @staticmethod
    ...
    def validate_zip_names(self, file_list):
        ...
        Schema(self.zip_schema).validate(file_list)
    ...
    def extensive_validation(self, file_list):
        ...
        5. Data rows cross relationships with the metadata
          ...
          :file_list: a list of AATAMS DM files
        """
        self.validate_zip_names(file_list)
        schema_name = 'metadata'
        metadata_file = [
            x for x in file_list if self.file2schema(x) == schema_name
        ]

        self.headers = {}
        self.headers['metadata'], self.metadata = self.validate_file(
            metadata_file[0])

        if self.cross_content_scope:
            self.report('\tPopulating cross content based on %s' %
                        metadata_file)
            ckeys = self.cross_content_scope.keys()
            self.cross_values = self.compute_cross_values(ckeys, self.metadata)
