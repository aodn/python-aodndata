...
class AnfogHandler(HandlerBase):
    ...
    def preprocess(self):
        ...
        """
        harvest_mission_file = os.path.join(
            self.resolve_params['relative_path_root'], AnfogFileClassifier.MISSION_LISTING)
        if self.file_type is FileType.ZIP:
            ...
            # Single NetCDF file upload
            # valid only for update of delayed mode file => Check that deployment exist in csv listing
            # set mission status will raise an error if deployment code not in listing
            ...
            set_mission_status(harvest_mission_file, deployment, platform, 'check')
            ...
            set_mission_status(harvest_mission_file, deployment, 'na', 'completed')
        else:
    ...
    def process_zip_dm(self):
        ...
        if len(is_dm_netcdf) != 1:
            ...
            if results:
                delete_previous_version(self, 'DM', destination, 'update')
            else:
                path_to_rt_folder = AnfogFileClassifier.dest_path(fv01, 'RT')
                delete_previous_version(self, 'DM', path_to_rt_folder, 'delayed_mode')
                deployment = AnfogFileClassifier.get_deployment_code(fv01)
                platform = AnfogFileClassifier.get_platform(fv01.name)
                set_mission_status(harvest_mission_file, deployment, platform, 'delayed _mode')
    ...
    def process_zip_rt(self):
        """
        Set destination path for realtimne file collection based on FV00 attribute
        ...
        if is_rt_netcdf is None or len(is_rt_netcdf) != 1:
            ...
            if response:  # directory exists, contains files that need to be deleted
                delete_previous_version(self, 'RT', destination, 'in_progress')
                ...
                set_mission_status(harvest_mission_file, deployment, platform, 'in_progress')
        ...
        for non_nc in non_nc_files:
            # Check type set to FORMAT_CHECK and publish type to UPLOAD_ONLY for ancillary files
...
def set_mission_status(filelisting, deployment, platform, message):
    """
    :return: Harvestmission.csv file with updated deployment status
    ...
    df = pandas.read_csv(filelisting, names=['deployment_name', 'platform_type', 'status'])
    ...
    df.to_csv(filelisting, mode='w', index=False, header=False)


def delete_previous_version(self, mode, path, mission_status):
    """
