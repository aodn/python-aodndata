...
SCAT_FILE_PATTERN = re.compile(
    r"""
                            ...
                            (?P<longitude>[0-9]{{3}})E-
                            DM00\.nc$
                            """.format("|".join(SCAT_VALID_SATS)),
    re.VERBOSE,
)

SAR_PREFIX_PATH = "IMOS/SRS/Surface-Waves/SAR"

SAR_FILE_PATTERN = re.compile(
    r"""
...
SAR_WIND_FILE_PATTERN = re.compile(
    r"""
...
def dest_path_alt_scat_common(filepath, file_pattern, prefix_path):
    ...
    return os.path.join(
        prefix_path, fields["platform_code"], coord_dirname, file_basename
    )


def dest_path_srs_surface_waves_sar(
    filepath, file_pattern=SAR_FILE_PATTERN, prefix_path=SAR_PREFIX_PATH
):
    file_basename = os.path.basename(filepath)
    if file_pattern.match(file_basename):
        fields = get_pattern_subgroups_from_string(file_basename, file_pattern)
...
class SrsSarWindHandler(HandlerBase):
    ...
    def preprocess(self):
        ...
        for nc_file in nc_files:
            png_output_path, png_output_dest_path = create_sar_wind_plot(
                nc_file.src_path, self.temp_dir
            )
    ...
    @staticmethod
    def dest_path(
        filepath, file_pattern=SAR_WIND_FILE_PATTERN, prefix_path=SAR_WIND_PREFIX_PATH
    ):
        file_basename = os.path.basename(filepath)
        if file_pattern.match(file_basename):
            fields = get_pattern_subgroups_from_string(file_basename, file_pattern)
...
def create_sar_wind_plot(netcdfFilePath, output_dir):
    ...
    levels = np.array(
        (0, 1, 2, 3, 4, 5, 6.5, 8, 9.5, 11, 12.5, 14, 15.5, 17.5, 19.5, 21.5, 25, 30)
    )
    ...
    with xr.open_dataset(netcdfFilePath, decode_times=False) as dset:
        ...
        v = uv[1]
        ax.barbs(
            dset["LONGITUDE"].isel(TIME=slc, RANGE=slc),
        ...
        img_output = tempfile.NamedTemporaryFile(
            delete=False, dir=output_dir, suffix=".png"
        )
        ...
        png_dest_path = "{}{}".format(
            os.path.splitext(SrsSarWindHandler.dest_path(netcdfFilePath))[0], ".png"
        )
