...
class MooringsFileClassifier(FileClassifier):
    ...
    VELOCITY_VAR = {'UCUR', 'VCUR', 'WCUR', 'UCUR_MAG', 'VCUR_MAG', 'CSPD', 'VEL1'}
...
class DwmFileClassifier(MooringsFileClassifier):
   ...
   #old pipeline stuff
    ...
    def dest_path(cls, input_file):
        ...
        if re.match('^.+FV02_hourly-depth-gridded-product', input_file_basename):
            dir_list.append(subfac)
            dir_list.append('CSIRO_gridded_all_variables')
        elif re.match('^.+FV03_daily-depth-gridded-product', input_file_basename):
            dir_list.append(subfac)
            dir_list.append('CSIRO_gridded_all_variables')
        elif subfac == 'DA':
            dir_list.append(subfac)
