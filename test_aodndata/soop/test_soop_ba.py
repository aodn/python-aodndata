...
TRANSECT_WITH_FREQUENCY = os.path.join(TEST_ROOT,
                                      'IMOS_SOOP-BA_AE_20170126T100839Z_VKAD_FV02_Antarctic-Discovery'
                                      '-ES60-38_END-20170201T144253Z_C-20181211T230659Z.nc')
TRANSECT_WITH_FREQUENCY_2 = os.path.join(TEST_ROOT,
                                      'IMOS_SOOP-BA_AE_20200820T205644Z_ZMTK_FV02_'
                                      'Tokatu-ES80-38_END-20200823T090247Z_C-20210709T025338Z.nc')
...
class TestSoopBaHandler(HandlerTestCase):
    ...
    def test_deployment_id_with_frequency(self, mock_callsign):
        handler = self.run_handler(TRANSECT_WITH_FREQUENCY)

        f = handler.file_collection[0]
        # self.assertEqual(f.check_type, PipelineFileCheckType.NC_COMPLIANCE_CHECK)
        self.assertEqual(f.publish_type, PipelineFilePublishType.HARVEST_UPLOAD)
        self.assertEqual(f.name, os.path.basename(TRANSECT_WITH_FREQUENCY))
        self.assertEqual(f.dest_path,
    ...
    @patch("aodndata.soop.soop_ba.ship_callsign_list", side_effect=mock_ship_callsign_list)
    def test_deployment_id_with_frequency_short_vessel_name(self, mock_callsign):
        # test the file processing of non compound name vessel (eg Tokatu, as opposed
        # to Antarctic-Discovery as in test test_deployment_id_with_frequency above)
        handler = self.run_handler(TRANSECT_WITH_FREQUENCY_2)

        f = handler.file_collection[0]
        # self.assertEqual(f.check_type, PipelineFileCheckType.NC_COMPLIANCE_CHECK)
        self.assertEqual(f.publish_type, PipelineFilePublishType.HARVEST_UPLOAD)
        self.assertEqual(f.name, os.path.basename(TRANSECT_WITH_FREQUENCY_2))
        self.assertEqual(f.dest_path,
                         'IMOS/SOOP/SOOP-BA/ZMTK_Tokatu/Tokatu_20200820-20200823_38/'
                         + f.name)
        self.assertTrue(f.is_stored)
